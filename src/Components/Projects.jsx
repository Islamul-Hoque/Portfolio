          ...
          <motion.div key={project.name} initial={{ opacity: 0, y: 60, scale: 0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} whileHover={{ scale: 1.03, boxShadow: "0px 8px 24px rgba(0,0,0,0.3)" }} transition={{ duration: 0.6, ease: "easeOut", type: "spring", bounce: 0.3 }} className="bg-gray-800 rounded-lg overflow-hidden relative flex flex-col">
            ...
            <div className="p-4 md:p-6 flex flex-col h-full">
              ...
              <div className="flex gap-2 mt-auto flex-wrap">
                <a href={project.live} target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-2 md:px-3 py-2 rounded transition duration-200"><FaExternalLinkAlt /> Live Demo</a>
                <button onClick={() => setSelectedProject(project)} className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-2 md:px-3 py-2 rounded transition duration-200"><FaEye /> View Details</button>
              </div>
            </div>
          </motion.div>
        ))}
        ...
        <motion.div className="fixed inset-0 bg-gray-700 bg-opacity-70 flex items-center justify-center z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <motion.div className="bg-gray-800 text-white rounded-lg p-8 w-[85%] md:w-[60%] relative text-left" initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }}>
            ...
            <div className="flex gap-4">
              <a href={selectedProject.code} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-gray-700 px-4 py-2 rounded hover:bg-gray-600 transition"><FaGithub /> Code</a>
              <a href={selectedProject.live} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-indigo-600 px-4 py-2 rounded hover:bg-indigo-700 transition"><FaExternalLinkAlt /> Live Demo</a>
            </div>
