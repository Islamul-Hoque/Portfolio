...
const Projects = () => {
  ...
  const filteredProjects = filter === "All" ? allProjects : allProjects.filter((project) => project.tag === filter);

  const filters = ["All", "React", "Next.js", "MERN"];

  return (
    <div id="projects" className="bg-gray-900 text-white min-h-screen px-8 md:px-16 py-20">
      <div className="mx-auto text-center">
        <motion.h2 initial={{ opacity: 0, y: -30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-3xl md:text-4xl font-bold text-indigo-400 mb-6">My Projects</motion.h2>
        <p className="text-gray-300 mb-10">Filter by technology stack to explore my best work</p>

        {/* Filter Buttons */}
        <div className="flex justify-center gap-4 flex-wrap mb-12">
          {filters.map((tag) => (
            <button key={tag} onClick={() => setFilter(tag)} className={`px-4 py-2 rounded-full font-medium transition duration-200 ${filter === tag ? "bg-indigo-500 text-white" : "bg-gray-700 text-gray-300 hover:bg-indigo-600 hover:text-white"}`}>{tag}</button>
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
          <AnimatePresence>
            {filteredProjects.map((project) => (
              <motion.div key={project.name} initial={{ opacity: 0, y: 60, scale: 0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} whileHover={{ scale: 1.03, boxShadow: "0px 8px 24px rgba(0,0,0,0.3)" }} transition={{ duration: 0.6, ease: "easeOut", type: "spring", bounce: 0.3 }} className="bg-gray-800 rounded-lg overflow-hidden relative flex flex-col">
                <div className="relative">
                  <img src={project.image} alt={project.name} className="w-full h-40 object-cover" />
                  <span className="absolute top-2 right-2 bg-indigo-500 text-white text-xs px-3 py-1 rounded-full shadow">{project.tag}</span>
                </div>
                <div className="p-4 md:p-6 flex flex-col h-full">
                  <h3 className="text-xl font-semibold text-indigo-300 mb-2">{project.name}</h3>
                  <p className="text-gray-300 mb-4 flex-grow">{project.description}</p>
                  <div className="flex gap-2 mt-auto flex-wrap">
                    <a href={project.live} target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-2 md:px-3 py-2 cursor-pointer rounded transition duration-200"><FaExternalLinkAlt /> Live Demo</a>
                    <button onClick={() => setSelectedProject(project)} className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-2 md:px-3 py-2 rounded cursor-pointer transition duration-200"><FaEye /> View Details</button>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        <AnimatePresence>
          {selectedProject && (
            <motion.div className="fixed inset-0 bg-gray-700 bg-opacity-70 flex items-center justify-center z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <motion.div className="bg-gray-800 text-white rounded-lg p-8 w-[85%] md:w-[60%] relative text-left" initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }}>
                ...
                <p className="text-gray-300 mb-4">{selectedProject.stack?.join(", ")}</p>
                <div className="flex gap-4">
                  <a href={selectedProject.code} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-gray-700 px-4 py-2 rounded hover:bg-gray-600 cursor-pointer transition"><FaGithub /> Code</a>
                  <a href={selectedProject.live} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-indigo-600 px-4 py-2 rounded hover:bg-indigo-700 cursor-pointer transition"><FaExternalLinkAlt /> Live Demo</a>
                </div>
                <button onClick={() => setSelectedProject(null)} className="absolute top-4 right-4 text-gray-400 hover:text-white"><IoCloseCircle size={24} /></button>
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>
